...
function Tree(name, options){
  ...
  var p = new Promise(function(yes,no){
    ...
    self.kDB = Sublevel(level(self.fullname, leveldbOpts ,function(err){
      if(err){
        no(err);
      }else{
        yes(self.db);
    ...
    self.db = self.kDB.sublevel('nontree');
    treeUtils.addMethods(self);
  });
  self.then = p.then.bind(p);
}
Tree.prototype = treeUtils.proto;
...
Tree.prototype.insertBbox =function(item){
  ...
  return all(extent.map(function(v){
    return this.get(v).catch(function(){
...
Tree.prototype.load = function(array){
  ...
  while(++i<len){
    ...
    batch[i] = {key:array[i].id,value:array[i],type:'put', prefix:self.kDB};
  ...
  Object.keys(bboxen).forEach(function(quad){
    var ids = bboxen[quad];
    things.push(this.get(quad).catch(function(){

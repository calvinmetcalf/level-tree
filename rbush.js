...
rbush.prototype = {
    ...
    search: function (bbox) {
        ...
        while (node) {
            for (i = 0, len = node.children.length; i < len; i++) {
                ...
                childBBox = node.leaf ? child : child.bbox;
    ...
    remove: function (item) {
        ...
        while (node || path.length) {
            ...
            if (node.leaf) { // check current node
                index = this.findItem(node.children,item);
    ...
    _insert: function (item, level, isNode) {
        ...
        var bbox = isNode ? item.bbox : item,
            insertPath = [];
    ...
    _allDistMargin: function (node, m, M, compare) {
        ...
        for (i = m; i < M - m; i++) {
            child = node.children[i];
            this._extend(leftBBox, node.leaf ? child : child.bbox);
            margin += this._margin(leftBBox);
        }

        for (i = M - m - 1; i >= m; i--) {
            child = node.children[i];
            this._extend(rightBBox, node.leaf ? child : child.bbox);
    ...
    _distBBox: function (node, k, p) {
        ...
        for (var i = k, child; i < p; i++) {
            child = node.children[i];
            this._extend(bbox, node.leaf ? child : child.bbox);
        }
